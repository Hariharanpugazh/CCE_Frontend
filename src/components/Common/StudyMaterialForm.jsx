...
const CategoryInput = ({ value, onChange, selectedType, onSelect }) => {
  ...
  useEffect(() => {
    ...
    if (token && inputValue && !isSuggestionSelected) {
      axios
        .get(
          `https://cce-backend.onrender.com/api/get-categories/?type=${selectedType}&query=${inputValue}`,
          {
            headers: {
...
const handleFileUpload = async (index, file) => {
  ...
  try {
    ...
    const response = await axios.post(
      "https://cce-backend.onrender.com/api/upload-drive-file/", // Backend endpoint for Drive upload
      formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export default function StudyMaterialForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://cce-backend.onrender.com/api/post-study-material/",
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
