...
export default function SidePreview({ selectedItem, handleViewItem, setSelectedItem, isSaved, fetchSavedJobs }) {
        ...
        try {
            ...
            if (selectedItem.job_data) {
                const res = await axios.post(
                    `https://cce-backend-kw0b.onrender.com/api/save-job/${selectedItem._id}/`,
                ...
                const res = await axios.post(
                    `https://cce-backend-kw0b.onrender.com/api/save-internship/${selectedItem.id}/`,
    ...
    const handleUnbookmark = async (event) => {
        ...
        try {
            ...
            if (selectedItem.job_data) {
                const res = await axios.post(
                    `https://cce-backend-kw0b.onrender.com/api/unsave-job/${selectedItem._id}/`,
                    { applicationId: selectedItem._id, userId }
                );
                if (res.status === 200) {
                    fetchSavedJobs()
                ...
                const res = await axios.post(
                    `https://cce-backend-kw0b.onrender.com/api/unsave-internship/${selectedItem.id}/`,
                    { applicationId: selectedItem.id, userId }
                );
                if (res.status === 200) {
                    fetchSavedJobs()
